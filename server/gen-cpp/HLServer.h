...
typedef struct _HLServer_retire_args__isset {
  _HLServer_retire_args__isset() : jobId(false) {}
  bool jobId;
} _HLServer_retire_args__isset;

class HLServer_retire_args {
 public:

  static const char* ascii_fingerprint; // = "56A59CE7FFAF82BCA8A19FAACDE4FB75";
  static const uint8_t binary_fingerprint[16]; // = {0x56,0xA5,0x9C,0xE7,0xFF,0xAF,0x82,0xBC,0xA8,0xA1,0x9F,0xAA,0xCD,0xE4,0xFB,0x75};

  HLServer_retire_args(const HLServer_retire_args&);
  HLServer_retire_args& operator=(const HLServer_retire_args&);
  HLServer_retire_args() : jobId(0) {
  }

  virtual ~HLServer_retire_args() throw();
  int64_t jobId;

  _HLServer_retire_args__isset __isset;

  void __set_jobId(const int64_t val);

  bool operator == (const HLServer_retire_args & rhs) const
  {
    if (!(jobId == rhs.jobId))
      return false;
    return true;
  ...
  bool operator < (const HLServer_retire_args & ) const;
...
class HLServer_retire_pargs {
 public:
  ...
  virtual ~HLServer_retire_pargs() throw();
  const int64_t* jobId;
...
class HLServerClient : virtual public HLServerIf {
 ...
 public:
  ...
  void retire(const int64_t jobId);
  void send_retire(const int64_t jobId);
 protected:
...
class HLServerProcessor : public ::apache::thrift::TDispatchProcessor {
 ...
 private:
  ...
  void process_retire(int32_t seqid, ::apache::thrift::protocol::TProtocol* iprot, ::apache::thrift::protocol::TProtocol* oprot, void* callContext);
 public:
  HLServerProcessor(boost::shared_ptr<HLServerIf> iface) :
    ...
    processMap_["retire"] = &HLServerProcessor::process_retire;
  }
