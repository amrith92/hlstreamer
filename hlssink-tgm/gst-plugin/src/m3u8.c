...
static gboolean gst_m3u8_update (GstM3U8 * m3u8, gchar * data,
    gboolean * updated);
static GstM3U8MediaFile * gst_m3u8_media_file_new (
    gchar * uri, gchar * title, GstClockTime duration,
    guint sequence);
...
/*
 * @data: a m3u8 playlist text data
 */
static gboolean
            ...
            if (list != NULL) {
                ...
                if (file->key) {
                    if (have_iv) {
                        ...
                        GST_WRITE_UINT32_BE (iv + 32, file->sequence);
                    }
                }
